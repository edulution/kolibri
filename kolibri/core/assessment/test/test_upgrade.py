...
from kolibri.core.assessment.upgrade import (
...
class ExamsUpgradeTestCase(TestCase):
    def test_resolve_conflicting_datasets_for_exams_and_related_models(self):
        ...
        ok_exam = ExamAssessment.objects.create(
            title="", question_count=1, collection=classroom, creator=coach
        )
        ok_assignment = ExamAssignmentAssessment.objects.create(
            exam=ok_exam, collection=classroom, assigned_by=coach
        )
        broken_exam = ExamAssessment.objects.create(
            title="", question_count=1, collection=classroom, creator=coach
        )
        broken_assignment = ExamAssignmentAssessment.objects.create(
