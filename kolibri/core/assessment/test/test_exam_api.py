...
class ExamAPITestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.exam = models.ExamAssessment.objects.create(
            title="title",
            question_count=1,
            active=True,
    ...
    def test_logged_in_admin_exam_create_with_assignments(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(
            models.ExamAssignmentAssessment.objects.get(collection=self.facility).exam,
            models.ExamAssessment.objects.get(id=exam_id),
        )
    ...
    def test_logged_in_admin_exam_create_with_learner_assignments(self):
        ...
        self.assertEqual(
            models.ExamAssignmentAssessment.objects.get(collection=adhoc_group).exam,
            models.ExamAssessment.objects.get(id=exam_id),
        )
    ...
    def test_logged_in_admin_exam_update_no_assignments(self):
        ...
        self.assertEqual(models.ExamAssessment.objects.get(id=exam_id).assignments.count(), 0)
    ...
    def test_logged_in_admin_exam_update_different_assignments(self):
        ...
        put_response = self.put_updated_exam(exam_id, exam)
        self.assertEqual(put_response.status_code, 200)
        assignments = models.ExamAssessment.objects.get(id=exam_id).assignments
    ...
    def test_logged_in_admin_exam_update(self):
        ...
        self.assertEqual(
            models.ExamAssessment.objects.get(id=self.exam.id).title, "updated title"
        )
    ...
    def test_logged_in_user_exam_no_update(self):
        ...
        self.assertEqual(response.status_code, 403)
        self.assertEqual(models.ExamAssessment.objects.get(id=self.exam.id).title, "title")
    ...
    def test_exam_with_no_counter_in_exercise_fails(self):
        ...
        response = self.post_new_exam(exam)
        self.assertEqual(response.status_code, 400)
        self.assertFalse(models.ExamAssessment.objects.filter(title=title).exists())
