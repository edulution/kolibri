...
from kolibri.core.assessment.models import ExamAssignmentAssessment
...
class ExamAssessmentSerializer(ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            obj = ExamAssessment.objects.get(title__iexact=title, collection=collection)
        except ExamAssessment.DoesNotExist:
            return attrs
    ...
    def to_internal_value(self, data):
        ...
        if "creator" not in data:
            if self.context["view"].action == "create":
                ...
                self.partial = True
        return super(ExamAssessmentSerializer, self).to_internal_value(data)

    def create(self, validated_data):
        collections = validated_data.pop("assignments")
        learners = validated_data.pop("learner_ids", [])
        new_exam = ExamAssessment.objects.create(**validated_data)
    ...
    def _create_exam_assignment(self, **params):
        return ExamAssignmentAssessment.objects.create(
    ...
    def _update_learner_ids(self, instance, learners):
        try:
            adhoc_group_assignment = ExamAssignmentAssessment.objects.select_related(
                "collection"
            ).get(exam=instance, collection__kind=ADHOCLEARNERSGROUP)
        except ExamAssignmentAssessment.DoesNotExist:
