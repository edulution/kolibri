...
def resolve_conflicting_datasets_for_exams_and_related_models():
    ...
    ExamAssessment.objects.exclude(collection__dataset_id=F("dataset_id")).update(
    ...
    assignment_sub_query = Subquery(
        ExamAssessment.objects.filter(pk=OuterRef("exam_id")).values("dataset_id")[:1],
        output_field=UUIDField(),
    )
    ExamAssignmentAssessment.objects.exclude(exam__dataset_id=F("dataset_id")).update(
