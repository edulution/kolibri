  ...
  return store.dispatch('loading').then(() => {
    return ChannelResource.fetchCollection({
      ...
      return showExamCreationPage(store, {
        ...
        pageName: PageNames.ASSESSMENT_CREATION_ROOT,
      });
    });
  });
}
export function showPracticeQuizCreationRootPage(store, params) {
  return fetchPracticeQuizzes().then(channels => {
    ...
    return showExamCreationPage(store, {
      classId: params.classId,
      contentList: channelContentList,
      pageName: PageNames.ASSESSMENT_CREATION_PRACTICE_QUIZ,
    });
  });
}
    ...
    return Promise.all(loadRequirements).then(([topicNode, childNodes]) => {
      return filterAndAnnotateContentList(childNodes).then(contentList => {
        ...
        return showExamCreationPage(store, {
          classId: params.classId,
          contentList,
          pageName: PageNames.ASSESSMENT_CREATION_SELECT_PRACTICE_QUIZ_TOPIC,
    ...
    return Promise.all(loadRequirements).then(([topicNode, childNodes]) => {
      return filterAndAnnotateContentList(childNodes).then(contentList => {
        ...
        return showExamCreationPage(store, {
          classId: params.classId,
          contentList,
          pageName: PageNames.ASSESSMENT_CREATION_TOPIC,
          ancestors: [...topicNode.ancestors, topicNode],
        });
      });
    });
    ...
    return Promise.all(loadRequirements).then(([topicNode, childNodes]) => {
      return filterAndAnnotateContentList(childNodes).then(() => {
        ...
        return showExamCreationPage(store, {
          ...
          pageName: PageNames.ASSESSMENT_CREATION_BOOKMARKS,
          ancestors: [...topicNode.ancestors, topicNode],
        });
      });
    });
  ...
  return store.dispatch('loading').then(() => {
    return Promise.all([_prepExamContentPreview(store, classId, contentId)])
      .then(([contentNode]) => {
        ...
        } else if (fromRoute && fromRoute.name === PageNames.ASSESSMENT_CREATION_TOPIC) {
          store.commit('SET_TOOLBAR_ROUTE', {
...
function _prepPracticeQuizContentPreview(store, classId, contentId) {
  return ContentNodeResource.fetchModel({ id: contentId }).then(
    contentNode => {
      ...
      store.commit('SET_PAGE_NAME', PageNames.ASSESSMENT_CREATION_PRACTICE_QUIZ_PREVIEW);
      return contentNode;
    },
    error => {
      return store.dispatch('handleApiError', { error, reloadOnReconnect: true });
...
function _prepExamContentPreview(store, classId, contentId) {
  return ContentNodeResource.fetchModel({ id: contentId }).then(
    contentNode => {
      ...
      store.commit('SET_PAGE_NAME', PageNames.ASSESSMENT_CREATION_PREVIEW);
      return contentNode;
    },
    error => {
      return store.dispatch('handleApiError', { error, reloadOnReconnect: true });
    ...
    return ContentNodeSearchResource.fetchCollection({
      ...
      return filterAndAnnotateContentList(results.results).then(contentList => {
        ...
        return showExamCreationPage(store, {
          ...
          pageName: PageNames.ASSESSMENT_CREATION_SEARCH,
          searchResults,
        });
      });
    });
...
const creationPages = [
  PageNames.ASSESSMENT_CREATION_ROOT,
  PageNames.ASSESSMENT_CREATION_TOPIC,
  PageNames.ASSESSMENT_CREATION_PREVIEW,
  PageNames.ASSESSMENT_CREATION_SEARCH,
  PageNames.ASSESSMENT_CREATION_BOOKMARKS,
  PageNames.ASSESSMENT_CREATION_BOOKMARKS_MAIN,
];
