...
class Edulution(KolibriPluginBase):
...
@register_hook
class EdulitionRedirect(RoleBasedRedirectHook):
    @property
    def roles(self):
    ...
    @property
    def url(self):
        return self.plugin_url(Edulution, "edulution")


@register_hook
class EdulutionNavItem(NavigationHook):
    bundle_id = "side_nav"
...
class EdulutionAsset(webpack_hooks.WebpackBundleHook):
    ...
    def plugin_data(self):
        ...
        return {
            ...
            "enableCustomChannelNav": conf.OPTIONS["Edulution"][
...
@register_hook
class EdulutionContentNodeHook(ContentNodeDisplayHook):
    def node_url(self, node):
        ...
        if kind_slug is not None:
            return (
                reverse("kolibri:kolibri.plugins.edulution:edulution")
                + "#/topics/"
                + kind_slug
                + node.id
            )
