<template>
  <div>
    <div style="padding-left: 16px; margin: 32px 0 8px 0">
      ...
      <TextTruncator v-if="topic.description" :text="topic.description" :maxHeight="90" :showTooltip="false" :showViewMore="true" dir="auto" class="page-description" />
    </div>
    <div v-if="contents[0].children.length">
      <ExpandableContentCardGroupGrid v-for="child in contents" :key="child.id" :child="child" />
    </div>
    <ContentCardGroupGrid v-if="!contents[0].children.length" :contents="contents" :genContentLink="genContentLink" style="padding-left: 16px; padding-top: 16px" />
